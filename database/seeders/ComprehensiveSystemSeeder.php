...
class ComprehensiveSystemSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Single admin account only (no sample customers)
    ...
    private function clearExistingData(): void
    {
        ...
        DB::table('users')->where('email', '!=', 'admin@chancelaptops.ae')->delete();
    ...
    private function createSystemUsers(): void
    {
        ...
        User::where('email', '!=', 'admin@chancelaptops.ae')->delete();
    ...
    private function createSampleOrders(array $customers): void
    {
        ...
        for ($i = 1; $i <= 150; $i++) {
            ...
            // Create order
            $order = Order::create([
                'order_number' => 'CLT' . str_pad($i, 6, '0', STR_PAD_LEFT),
                'user_id' => $customer->id,
    ...
    private function displaySummary(): void
    {
        ...
        $this->command->info('=====================================');
        $this->command->info('👥 Users: ' . User::count() . ' (admins: ' . User::where('role', 'admin')->count() . ')');
        $this->command->info('📦 Orders: ' . Order::count());
        $this->command->info('=====================================');
        $this->command->info('');
        $this->command->info('🔐 ADMIN LOGIN:');
        $this->command->info('Email: admin@chancelaptops.ae');
        $this->command->info('Password: ' . AdminUserSeeder::ADMIN_PASSWORD);
        $this->command->info('');
    }
}
