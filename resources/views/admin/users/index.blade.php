    ...
    <div class="bg-gray-50 rounded-lg p-6">
        <form method="GET" action="{{ route('admin.users.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="role" class="w-full bg-gray-700 border border-gray-300 rounded-lg px-4 py-2 text-gray-900 focus:outline-none focus:border-primary-500">
            ...
            <div>
                ...
                <select name="status" class="w-full bg-gray-700 border border-gray-300 rounded-lg px-4 py-2 text-gray-900 focus:outline-none focus:border-primary-500">
    ...
    <div class="bg-gray-50 rounded-lg p-4">
        <form id="bulkActionForm" method="POST" action="{{ route('admin.users.bulk-action') }}">
            ...
            <div class="flex flex-wrap items-center gap-4">
                <div class="flex items-center">
                    <input type="checkbox" id="selectAll" class="mr-2 rounded border-slate-200 bg-gray-700 text-primary-500 focus:ring-primary-500">
                    <label for="selectAll" class="text-sm text-gray-700">Select All</label>
                </div>
                
                <select name="action" id="bulkAction" class="bg-gray-700 border border-gray-300 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-primary-500">
    ...
    <div class="bg-gray-50 rounded-lg overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">
                            <input type="checkbox" id="selectAllTable" class="rounded border-slate-200 bg-gray-700 text-primary-500 focus:ring-primary-500">
                        </th>
                ...
                <tbody class="bg-gray-50 divide-y divide-gray-700">
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-100">
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($user->id !== auth()->id())
                                    <input type="checkbox" name="selected_users[]" value="{{ $user->id }}" class="user-checkbox rounded border-slate-200 bg-gray-700 text-primary-500 focus:ring-primary-500">
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-700">{{ $user->phone ?: 'N/A' }}</div>
                                @if($user->last_login_at)
                                    <div class="text-sm text-gray-600">Last: {{ $user->last_login_at->diffForHumans() }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
