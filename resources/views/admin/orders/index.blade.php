...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex space-x-3">
            ...
            <a href="{{ route('admin.orders.statistics') }}" 
               class="px-4 py-2 bg-gray-50 text-white rounded-lg hover:bg-gray-100 transition-colors">
    ...
    <div class="bg-gradient-to-r from-white to-gray-50 rounded-xl border border-gray-200 p-4">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('admin.orders.index', ['status' => 'pending', 'view_status' => 'unread']) }}" 
               class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors
                      {{ request('status') === 'pending' && request('view_status') === 'unread' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-gray-700/50 text-gray-700 hover:bg-red-500/10 hover:text-red-400' }}">
            ...
            <a href="{{ route('admin.orders.index', ['status' => 'confirmed']) }}" 
               class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors
                      {{ request('status') === 'confirmed' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-gray-700/50 text-gray-700 hover:bg-red-500/10 hover:text-red-400' }}">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @endphp
                @if($confirmedCount > 0)
                    <span class="ml-2 px-2 py-1 bg-red-500/70 text-white text-xs rounded-full">{{ $confirmedCount }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}" 
               class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors
                      {{ request('status') === 'pending' && !request('view_status') ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' : 'bg-gray-700/50 text-gray-700 hover:bg-yellow-500/10 hover:text-yellow-400' }}">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.orders.index', ['status' => 'processing']) }}" 
               class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors
                      {{ request('status') === 'processing' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' : 'bg-gray-700/50 text-gray-700 hover:bg-purple-500/10 hover:text-purple-400' }}">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.orders.index', ['payment_status' => 'pending']) }}" 
               class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors
                      {{ request('payment_status') === 'pending' ? 'bg-orange-500/20 text-orange-400 border border-orange-500/30' : 'bg-gray-700/50 text-gray-700 hover:bg-orange-500/10 hover:text-orange-400' }}">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Clear All Filters -->
            <div class="border-l border-slate-200 pl-3 ml-2">
                <a href="{{ route('admin.orders.index') }}" 
                   class="inline-flex items-center px-3 py-2 bg-gray-600/50 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-500/50 transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Filters -->
    <div class="bg-gradient-to-r from-white to-gray-50 rounded-xl border border-gray-200 p-6">
        <form method="GET" action="{{ route('admin.orders.index') }}">
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    </div>

                    <!-- Status Filter -->
                    <div>
                        <label for="status" class="block text-sm font-medium text-gray-700 mb-2">Status</label>
                        <select id="status" name="status" class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    <div>
                        ...
                        <select id="payment_status" name="payment_status" class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                            <option value="">All Status</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <!-- View Status -->
                    <div>
                        <label for="view_status" class="block text-sm font-medium text-gray-700 mb-2">View Status</label>
                        <select id="view_status" name="view_status" class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    <div>
                        ...
                        <select id="date_preset" class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    <div class="flex items-end space-x-2">
                        <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors text-sm font-medium flex items-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(request()->hasAny(['search', 'status', 'payment_status', 'payment_method', 'view_status', 'date_from', 'date_to']))
                            <a href="{{ route('admin.orders.index') }}" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors text-sm flex items-center">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                                Clear
                ...
                @if(request()->hasAny(['search', 'status', 'payment_status', 'payment_method', 'view_status', 'date_from', 'date_to']))
                    <div class="flex flex-wrap gap-2 pt-2 border-t border-gray-300">
                        ...
                        @if(request('search'))
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-red-900/50 text-red-200 border border-red-700">
                                Search: "{{ request('search') }}"
                                <a href="{{ route('admin.orders.index', request()->except('search')) }}" class="ml-1 text-red-300 hover:text-red-100">×</a>
                            </span>
                        @endif
    ...
    <!-- Orders Table -->
    <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 overflow-hidden">
        ...
        <form id="bulk-form" action="{{ route('admin.orders.bulk-action') }}" method="POST">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50/20">
                <div class="flex items-center space-x-4">
                    <label class="flex items-center">
                        <input type="checkbox" id="select-all" class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded bg-white">
                        <span class="ml-2 text-sm text-gray-700">Select All</span>
                    </label>
                    
                    <select name="action" id="bulk-action-select" class="px-3 py-1 bg-white border border-gray-300 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    <select name="bulk_status" id="bulk-status-select" class="px-3 py-1 bg-white border border-gray-300 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500" style="display: none;">
                    ...
                    <button type="submit" id="bulk-submit-btn" class="px-4 py-1 bg-red-500 text-white rounded hover:bg-red-600 transition-colors text-sm font-medium">
                        Apply
                    </button>
                    
                    <span id="selected-count" class="text-sm text-gray-600 ml-4" style="display: none;">
                ...
                <table class="w-full">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">
                                <input type="checkbox" class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded bg-white">
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($orders as $order)
                            <tr class="hover:bg-gray-100/30 transition-colors {{ !$order->isViewedByAdmin() ? 'bg-red-900/20 border-l-4 border-red-500' : '' }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-2">
                                        <input type="checkbox" name="selected_orders[]" value="{{ $order->id }}" class="order-checkbox h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded bg-white">
                                        @if(!$order->isViewedByAdmin())
                                            <div class="w-2 h-2 bg-red-500 rounded-full animate-pulse" title="Unviewed order"></div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-3">
                                        <div>
                                            <div class="flex items-center space-x-2">
                                                <div class="text-sm font-medium {{ !$order->isViewedByAdmin() ? 'text-red-200 font-bold' : 'text-white' }}">
                                                    {{ $order->order_number }}
                                                </div>
                                                @if(!$order->isViewedByAdmin())
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-red-900 text-red-200 border border-red-700">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @elseif($order->payment_method === 'bank_transfer')
                                            ...
                                            @if($order->transfer_slip_path)
                                                <span class="text-xs text-red-400 font-medium" title="Transfer slip uploaded">📎</span>
                                            @else
