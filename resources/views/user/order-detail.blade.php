...
@section('title', 'Order Details - ' . $order->order_number . ' - CHANCE LAPTOPS')
@section('description', 'View detailed information about your CHANCE LAPTOPS order including items, shipping, and payment details.')
                ...
                <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                            <div class="text-center p-4 bg-gray-50/30 rounded-lg">
                                <div class="text-2xl font-bold text-gray-900 mb-1">{{ $order->formatted_total }}</div>
                            ...
                            <div class="text-center p-4 bg-gray-50/30 rounded-lg">
                                <div class="text-lg font-semibold text-gray-900 mb-1">{{ $order->orderItems->count() }}</div>
                                <div class="text-sm text-gray-600">{{ Str::plural('Item', $order->orderItems->count()) }}</div>
                            </div>
                        </div>
                    ...
                    <div class="p-6">
                        <div class="space-y-4">
                            @foreach($order->orderItems as $item)
                                <div class="flex items-center space-x-4 p-4 bg-gray-50/20 rounded-lg">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center space-x-4 mt-2 text-sm">
                                            ...
                                            <span class="text-gray-600">Unit Price: {{ $item->formatted_unit_price }}</span>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-lg font-semibold text-gray-900">{{ $item->formatted_total_price }}</div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="p-6">
                            <div class="space-y-4">
                                @foreach($order->transactions as $transaction)
                                    <div class="flex items-center justify-between p-4 bg-gray-50/20 rounded-lg">
                                        <div>
                                            <div class="font-medium text-white">{{ ucfirst($transaction->payment_method) }}</div>
                                            <div class="text-sm text-gray-600">{{ $transaction->transaction_id }}</div>
                                            ...
                                            <div class="text-sm">
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                                    {{ $transaction->status === 'completed' ? 'bg-green-100 text-green-800' : 
                                                       ($transaction->status === 'pending' ? 'bg-red-100 text-red-800' : 'bg-red-100 text-red-800') }}">
                ...
                <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-3">
                        <a href="{{ route('orders.invoice', $order->order_number) }}" 
                           class="w-full inline-flex items-center justify-center px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors font-medium">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if($order->status === 'delivered')
                            <form action="{{ route('user.orders.reorder', $order) }}" method="POST" class="w-full">
                                @csrf
                                <button type="submit" 
                                        class="w-full inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors font-medium">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if($order->canBeCancelled())
                            <button type="button" 
                                    ...
                                    class="w-full inline-flex items-center justify-center px-4 py-2 border border-red-600 text-red-400 rounded-lg hover:bg-red-600 hover:text-red-600 transition-colors font-medium">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="cancelContactModal" class="fixed inset-0 bg-gray-900/40 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 max-w-md w-full">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900">Request Order Cancellation</h3>
                <button onclick="hideCancelContactInfo()" class="text-gray-600 hover:text-red-600">
        ...
        <div class="p-6">
            <div class="mb-4">
                <p class="text-gray-700 mb-4">To cancel your order <strong class="text-white">{{ $order->order_number }}</strong>, please contact our customer service team:</p>
            </div>
            
            <div class="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                <div class="text-center">
                    <h4 class="font-bold text-gray-900 text-lg mb-2">CHANCE LAPTOPS</h4>
                    <p class="text-sm text-gray-700">Contact our team to request cancellation</p>
                </div>
            </div>

            <div class="space-y-4">
                <!-- Phone Contact -->
                <div class="flex items-center space-x-3 p-3 bg-gray-50/30 rounded-lg">
                ...
                <div class="flex items-center space-x-3 p-3 bg-gray-50/30 rounded-lg">
                    ...
                    <div>
                        ...
                        <a href="mailto:info@chancelaptops.ae?subject=Order Cancellation Request - {{ $order->order_number }}&body=Hi, I would like to cancel my order {{ $order->order_number }}. Please process this request and confirm." 
                           class="text-red-400 hover:text-red-300 text-sm">info@chancelaptops.ae</a>
                        <div class="text-xs text-gray-600">Expert support</div>
                    </div>
                </div>
            </div>
