...
<div class="min-h-screen bg-white">
    ...
    <section class="relative bg-gradient-to-r from-white to-gray-50 border-b border-gray-200/30 py-8">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="lg:w-64 flex-shrink-0">
                            ...
                            <div class="flex gap-2">
                                    <input type="text" 
                                       ...
                                       placeholder="Search products..." 
                                           class="flex-1 bg-white border border-gray-300 text-gray-900 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm">
                                    <button type="submit" 
                    ...
                    <!-- Categories Filter -->
                    <div class="mb-6">
                            <h4 class="text-md font-medium text-gray-700 mb-3">Categories</h4>
                        <div class="space-y-2">
                            <a href="{{ route('products.search', ['q' => $searchTerm]) }}" 
                                   class="flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-all {{ !request('category') ? 'bg-primary-500 text-black' : 'text-gray-700 hover:bg-primary-500/10 hover:text-primary-400' }}">
                            ...
                            @foreach($categories as $category)
                                    <a href="{{ route('products.search', ['q' => $searchTerm, 'category' => $category->slug ?: $category->id]) }}" 
                                       class="flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-all {{ request('category') == ($category->slug ?: $category->id) ? 'bg-primary-500 text-black' : 'text-gray-700 hover:bg-primary-500/10 hover:text-primary-400' }}">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Sort Options -->
                    <div>
                            <h4 class="text-md font-medium text-gray-700 mb-3">Sort By</h4>
                        <form method="GET" action="{{ route('products.search') }}" id="sortForm">
                            ...
                            @endif
                                <select name="sort" 
                                        class="w-full bg-white border border-gray-300 text-gray-900 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm" 
                    ...
                    <div>
                        ...
                        @foreach($products as $product)
                                <a href="{{ route('products.show', ['category' => $product->category ? ($product->category->slug ?: $product->category->id) : $product->category_id, 'product' => $product->slug ?: $product->id]) }}" 
                                   class="group bg-white rounded-xl border border-gray-200/30 overflow-hidden hover:border-primary-500/30 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-primary-500/10">
                                    ...
                                    <div class="relative overflow-hidden bg-white aspect-square">
                                        ...
                                        <div class="absolute top-3 left-3 flex flex-col gap-1">
                                            ...
                                            @if($product->is_on_sale)
                                                <span class="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow">Sale</span>
                                            @endif
                                        </div>
                                    ...
                                    <div class="absolute top-3 right-3">
                                        @if($product->status && in_array($product->status->status_name, ['Coming Soon', 'Pre Order']))
                                            <div class="bg-violet-600/90 text-white text-xs font-medium px-2 py-1 rounded-lg backdrop-blur-sm">
                                        ...
                                        @elseif($product->is_in_stock_uae)
                                            <div class="bg-sky-600/90 text-white text-xs font-medium px-2 py-1 rounded-lg backdrop-blur-sm">
                                                In Stock UAE
                                            </div>
                                        @else
                                    ...
                                    <!-- Product Info -->
                                    <div class="p-4">
                                        <h3 class="text-sm font-semibold text-gray-900 mb-3 line-clamp-2 group-hover:text-primary-400 transition-colors">
                                        ...
                                        <div class="mt-auto">
                                            @if($product->can_add_to_cart)
                                                <button onclick="event.preventDefault(); event.stopPropagation(); addToCartFromSearch({{ $product->id }}, '{{ addslashes($product->name) }}')" 
                                                        class="w-full bg-gradient-to-r from-primary-500/15 to-red-500/15 backdrop-blur-sm border border-primary-400/40 hover:from-primary-500/25 hover:to-red-500/25 hover:border-primary-400/60 text-primary-200 px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 shadow-lg hover:shadow-primary-500/20 hover:-translate-y-0.5 tracking-wide">
                                                Add to Cart
                                                    </button>
                                                @else
                                                    <button class="w-full bg-gray-600/50 text-gray-600 px-4 py-2.5 rounded-lg text-sm font-semibold cursor-not-allowed" 
                ...
                @else
                    ...
                    <div class="text-center py-16">
                        ...
                        <h3 class="text-xl font-semibold text-gray-900 mb-2">No Results Found</h3>
                            <p class="text-gray-600 mb-6">Sorry, we couldn't find any products matching "<span class="text-primary-400">{{ $searchTerm }}</span>" that are currently in stock.</p>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 justify-center mt-6">
                                    ...
                                    <a href="{{ route('home') }}" 
                                       class="inline-flex items-center px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-lg transition-all border border-gray-300">
                                        Back to Home
                                    </a>
                            </div>
                        </div>
