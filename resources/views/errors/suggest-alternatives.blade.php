...
@section('content')
<div class="min-h-screen bg-gradient-to-b from-white to-white py-20">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16">
            ...
            <p class="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed mb-4">
                We couldn't find the page you're looking for: <span class="text-red-600 font-mono">{{ $path }}</span>
            </p>
        ...
        <div class="grid md:grid-cols-2 gap-8 mb-12">
            ...
            <div class="bg-slate-100/50 rounded-2xl p-6 border border-gray-200/30">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('categories.show', $category->slug ?: $category->id) }}" 
                       class="block p-3 rounded-lg bg-slate-200/50 hover:bg-slate-200/50 transition-colors group">
                        <div class="text-gray-900 font-medium group-hover:text-red-600 transition-colors">
                            {{ $category->name }}
                        </div>
                        @if($category->slug)
        ...
        <div class="text-center space-y-4">
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('home') }}" 
                   class="inline-flex items-center px-6 py-3 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600 transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('categories.index') }}" 
                   class="inline-flex items-center px-6 py-3 bg-transparent text-white border border-gray-300 font-semibold rounded-lg hover:bg-gray-100 transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M4 6h16v2H4zm0 5h16v2H4zm0 5h16v2H4z"/>
                    </svg>
                    Browse Categories
            ...
            <!-- Search Alternative -->
            <div class="mt-8 p-6 bg-slate-100/30 rounded-xl border border-gray-200/20">
                <h3 class="text-lg font-semibold text-gray-900 mb-3">Can't find what you're looking for?</h3>
                <form action="{{ route('products.search') }}" method="GET" class="flex gap-3">
                    <input type="text" 
                           name="q" 
                           placeholder="Search for products..." 
                           value="{{ $requestedProduct ? str_replace('-', ' ', $requestedProduct) : '' }}"
                           class="flex-1 px-4 py-3 bg-slate-200 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-red-300">
                    <button type="submit" 
