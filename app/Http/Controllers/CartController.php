...
class CartController extends Controller
{
    public function index()
    {
        ...
        $isWebXPayRedirect = !$isFromOurWebXPayHandler && (
                           ...
                           (str_contains($referer, 'xpay') && !str_contains($referer, 'chancelaptops.ae'))
                           );
        ...
        if ($isWebXPayRedirect) {
            \Log::error('🚨 WEBXPAY REDIRECT TO CART DETECTED!', [
                ...
                'detection_reason' => [
                    ...
                    'external_xpay_in_referer' => (str_contains($referer, 'xpay') && !str_contains($referer, 'chancelaptops.ae'))
                ],
                'loop_prevention' => [
    ...
    public function add(Request $request)
    {
        try {
            ...
            $product = SmaProduct::with('status')->find($request->product_id);
        ...
        if (!$product->can_add_to_cart) {
            return response()->json([
                'success' => false,
                'message' => $product->cart_restriction_reason ?: 'This product cannot be added to cart'
            ]);
        }
