...
class PaymentController extends Controller
{
    ...
    public function handleWebXPayReturn(Request $request)
    {
        try {
            ...
            if (str_contains($referer, '/cart') && str_contains($referer, 'chancelaptops.ae')) {
    ...
    public function initiateKokoPayPayment(Request $request, Order $order)
    {
        try {
            ...
            $customerData = [
                ...
                'firstName' => $firstName,
                'lastName' => $lastName,
                'email' => $order->customer_email ?: 'customer@chancelaptops.ae',
                'contactNumber' => $order->customer_phone,
                'description' => 'Chance Laptops Order #' . $order->id,
            ];
