...
class SmaProduct extends Model
{
    ...
    protected $casts = [
        ...
        'wholesale' => 'boolean',
        'in_stock_uae' => 'boolean',
    ];

    public const WHOLESALE_MIN_QTY = 5;
    /**
    ...
    public function getMainImageAttribute()
    {
        if ($this->image) {
            ...
            return config('products.image_base_url') . $this->image;
        }
        
        // Use eager-loaded photos when available to avoid N+1 queries
        ...
        if ($firstPhoto) {
            ...
            return config('products.image_base_url') . $firstPhoto->photo;
        }
    ...
    public function getImagesAttribute()
    {
        ...
        if ($this->image) {
            if (str_starts_with($this->image, 'http')) {
                ...
                $images[] = config('products.image_base_url') . $this->image;
            }
        }
        ...
        foreach ($this->photos as $photo) {
            if (str_starts_with($photo->photo, 'http')) {
                ...
                $images[] = config('products.image_base_url') . $photo->photo;
            }
        }
    ...
    /**
     * Scope for wholesale products
     */
    public function scopeWholesale($query)
    {
        return $query->where('wholesale', 1);
    }

    /**
     * Scope for retail (non-wholesale) products
     */
    public function scopeRetail($query)
    {
        return $query->where(function ($q) {
            $q->where('wholesale', 0)->orWhereNull('wholesale');
        });
    }

    /**
     * Whether product is flagged wholesale
     */
    ...
    /**
     * Whether product is flagged In Stock UAE
     */
    public function getIsInStockUaeAttribute(): bool
    {
        return (bool) $this->in_stock_uae;
    }

    /**
     * Digits-only WhatsApp number used for purchase enquiries.
     */
    ...
    /**
     * Public product URL for WhatsApp messages.
     */
    ...
    /**
     * WhatsApp deep link for wholesale enquiry (does not affect checkout).
     */
    public function wholesaleWhatsappUrl(): string
    {
        $min = (int) config('products.wholesale_min_units', self::WHOLESALE_MIN_QTY);
        $message = "Hi Chance Laptops, I'd like to purchase this wholesale product:\n"
            ."{$this->name}\n"
            ."Code: {$this->code}\n"
            ."Link: {$this->publicUrl()}\n\n"
            ."Note: Minimum order is {$min} units across any products.";

        return 'https://wa.me/'.$this->whatsapp_number.'?text='.rawurlencode($message);
    }

    /**
     * WhatsApp deep link for In Stock UAE enquiry (does not affect checkout).
     */
    public function uaeWhatsappUrl(): string
    {
        $message = "Hi Chance Laptops, I'd like to purchase this In Stock UAE product:\n"
            ."{$this->name}\n"
            ."Code: {$this->code}\n"
            ."Link: {$this->publicUrl()}\n\n"
    ...
    /**
     * Available quantity for cart (local stock only — UAE/wholesale use WhatsApp).
     */
    ...
    public function getCanAddToCartAttribute()
    {
        ...
        return $this->hide == 0 && $this->stock_quantity > 0;
    }
    ...
    public function getCartRestrictionReasonAttribute()
    {
        ...
        if ($this->stock_quantity <= 0) {
            if ($this->is_wholesale && $this->is_in_stock_uae) {
                return 'Wholesale / In Stock UAE — WhatsApp only';
            }
            if ($this->is_wholesale) {
                return 'Wholesale — WhatsApp only';
            }
            if ($this->is_in_stock_uae) {
                return 'In Stock UAE — WhatsApp only';
            }
